...
function draw() {
  ...
  for (let i = 0; i < rectangle.length; i++) {
    ...
    if (
      ...
      player.x > rectangle[i].x &&
      player.y + player.h > rectangle[i].y &&
      player.y < rectangle[i].y + rectangle[i].h
    ) {
      (player.x = rectangle[i].x + rectangle[i].w), (player.y = rectangle[i].y);
    }

    if (
      player.x + player.w > rectangle[i].x &&
      player.x + player.w < rectangle[i].x + rectangle[i].w &&
      player.y + player.h < rectangle[i].y + rectangle[i].h &&
      player.y + player.h > rectangle[i].y
    ) {
      (player.x = rectangle[i].x), (player.y = rectangle[i].y + rectangle[i].h);
    }
  }
}
